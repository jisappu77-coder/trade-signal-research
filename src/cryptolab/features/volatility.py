...
def ewm_stdev(
    returns: str = "log_return", *, halflife: int = 72, alias: str = "sigma"
) -> pl.Expr:
...
def realised_vol(
    returns: str = "log_return", *, window_bars: int, alias: str = "realised_vol"
) -> pl.Expr:
...
def drawdown_from_high(
    ...
    return (
        (pl.col(close) / pl.col(close).rolling_max(window_bars, min_samples=1)) - 1.0
    ).alias(alias)
